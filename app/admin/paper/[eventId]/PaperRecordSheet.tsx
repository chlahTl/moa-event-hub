...
export default function PaperRecordSheet({ event, booths: initialBooths }: { event: PaperEvent; booths: Booth[] }) {
  const [title, setTitle] = useState(`${event.name} 참가 기록지`);
  const [subtitle, setSubtitle] = useState(event.institution);
  const [rowCount, setRowCount] = useState(event.stampEnabled ? 6 : 8);
    ...
    <main className="paper-page-shell">
      <aside className="paper-controls" aria-label="기록지 설정">
        <div className="paper-controls-heading">
          ...
          <div><span>종이 접수 양식</span><h1>인쇄 항목 설정</h1></div>
        </div>
